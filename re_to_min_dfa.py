...
# dfa -> min dfa
def minimizable(state1, state2):
    list1 = state1.get_list()
    list2 = state2.get_list()
    set1 = set(list1)
    set2 = set(list2)
    set12 = set1.intersection(set2)
    set1 = set1.difference(set12)
    set2 = set2.difference(set12)
    set1 = set1.union(set2)
    # contains uncommon stuff between s1 and s2
    list12 = list(set1)
    for link in list12:
        if link[0] == state1.get_state() or link[0] == state2.get_state():
            continue
        else:
            return False
    return True

# Find Non-distinguishable states by Hopcroft's algorithm and join them
for state1 in list_of_states:
    for state2 in list_of_states:
        if minimizable(state1, state2) and state1.get_state() != state2.get_state():
            if state1.get_state() != death_state.get_state() and state2.get_state() != death_state.get_state():
                join_states(state1, state2)
